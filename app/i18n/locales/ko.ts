...
export const ko = {
  reviewSummary: {
    ...
    criticalError: {
      message:
        "⚠ 리뷰 파이프라인이 치명적인 오류로 실패했습니다. GitLab 작업을 확인하고 이 작업을 다시 시도하세요.",
      messageWithLinks: ({ linkToJobDetail }: { linkToJobDetail: string }) =>
        `⚠ 리뷰 파이프라인이 치명적인 오류로 실패했습니다. [**파이프라인 세부 정보를 확인**](${linkToJobDetail})하고 이 작업을 다시 시도하세요.`,
    },
  },
  reviewProcess: {
    reviewingMarker: {
      ...
      manualDeleteHint: ({ linkToJobDetail }: { linkToJobDetail: string }) =>
        `[**리뷰 프로세스**](${linkToJobDetail})가 멈춘 것처럼 보이면 이 댓글을 수동으로 삭제해 차단을 해제할 수 있습니다. 다만 먼저 최신 리뷰 CI 워크플로 상태를 확인해서 아직 실행 중인지, 또는 다시 실행해야 하는지 확인하세요.`,
      queueNotice: {
