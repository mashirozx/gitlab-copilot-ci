...
export const zhLzh = {
  reviewSummary: {
    ...
    criticalError: {
      message: "⚠ 审视流水失于重错。请察 GitLab 作业，并重试此作业。",
      messageWithLinks: ({ linkToJobDetail }: { linkToJobDetail: string }) =>
        `⚠ 审视流水失于重错。请[**察流水详情**](${linkToJobDetail})，并重试此作业。`,
    },
  },
  reviewProcess: {
    reviewingMarker: {
      ...
      manualDeleteHint: ({ linkToJobDetail }: { linkToJobDetail: string }) =>
        `若 [**评审流程**](${linkToJobDetail}) 若滞，可手动删此评论以解阻。然宜先察最近之 review CI workflow，确认其仍在运行，或于必要时重启之。`,
      queueNotice: {
